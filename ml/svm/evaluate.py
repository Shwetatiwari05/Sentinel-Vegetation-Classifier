...
def main():
    ...
    logger.info("Loading 1D pixel dataset...")
    features = []
    labels = []
    ...
    metrics_path = os.path.join(OUTPUT_DIR, 'svm_evaluation_metrics.json')
    with open(metrics_path, 'w') as f:
        json.dump(metrics, f, indent=4)
    logger.info(f"Saved SVM metrics to {metrics_path}")
    
    # Generate and save text report
    report = classification_report(y_test, y_pred, target_names=['Non-Grass', 'Grass'])
    report_path = os.path.join(OUTPUT_DIR, 'svm_classification_report.txt')
    ...
    plt.savefig(cm_path)
    plt.close()
    logger.info(f"Saved SVM confusion matrix plot to {cm_path}")
